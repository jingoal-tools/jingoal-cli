...
function isToContinue (root, projectName, rnPackage){
  prompt.start();

  var property = {
    name: 'yesno',
    message: 'node_modules 文件夹已经被删除，是否继续？',
    validator: /y[es]*|n[o]?/,
    warning: 'Must respond yes or no',
    default: 'no'
  };
  ...
  prompt.get(property, function (err, result) {
    if (result.yesno[0] === 'y') {
      installProject(root, projectName, rnPackage);
    } else {
      console.log('Project initialization canceled');
      process.exit();
    }
...
function run(root, projectName, rnPackage) {
  // console.log(getInstallPackage(rnPackage))
  exec('rimraf node_modules', function(e, stdout, stderr) {
    if (e) {
      console.log(stdout);
      console.error(stderr);
      console.error('`rimraf node_modules ` failed');
      process.exit(1);
    }

    isToContinue(root, projectName, rnPackage);
  })
}

function installProject (root, projectName, rnPackage){
